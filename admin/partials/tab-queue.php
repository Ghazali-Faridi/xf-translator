...
<div class="api-translator-section">
    ...
    <div style="margin-top: 20px; padding: 15px; background: #fff; border-left: 4px solid #dc3232;">
        ...
        <p style="margin: 10px 0; color: #666;">
            <?php _e('Reset all failed translation jobs back to "pending" status so they can be retried by the workers.', 'xf-translator'); ?>
            <?php if ($failed_count > 0) : ?>
                <strong><?php echo esc_html(sprintf(__('Currently: %d failed job(s).', 'xf-translator'), $failed_count)); ?></strong>
            <?php endif; ?>
        </p>
        <form method="post" action="" style="margin-top: 10px;">
            <?php wp_nonce_field('api_translator_settings', 'api_translator_nonce'); ?>
            <input type="hidden" name="api_translator_action" value="reset_failed_queue">
            <button type="submit" class="button button-secondary" onclick="return confirm('<?php echo esc_js(__('Reset all failed jobs back to pending status? They will be retried by the workers.', 'xf-translator')); ?>');">
